...
public sealed class ClusterPackagePublisherTests
{
    ...
    public async Task PublishAsync_CreatesPackageWithRefreshedReadOnlySasAndUploadsIt()
    {
        ...
        try
        {
            ...
            var sasUri =
                ...
                "&sig=secret";
            var runner = new PackageRunner(sasUri, capturedArchive);
            ...
            Assert.Equal(AzCopyProcessMode.SensitiveCaptured, runner.Calls[0].Mode);
            var sasArguments = runner.Calls[0].Arguments;
            ...
            Assert.Equal("copy", runner.Calls[1].Arguments[0]);
            Assert.Equal(
                "https://account123.blob.core.windows.net/container/cluster_package.zip",
                runner.Calls[1].Arguments[2]);
            Assert.Equal("AZCLI", runner.Calls[1].Environment?["AZCOPY_AUTO_LOGIN_TYPE"]);
    ...
    public async Task PublishAsync_RejectsDeviceCodeBecauseItCannotGenerateDelegationSas()
    {
        ...
        try
        {
            var runner = new PackageRunner(string.Empty, Path.Combine(directory.FullName, "unused.zip"));
            var publisher = new ClusterPackagePublisher(runner);
    ...
    private sealed class PackageRunner(string sasUri, string capturedArchive) : IAzCopyRunner
    {
